...
st.markdown("""
    ...
    /* Usunięcie zbędnych odstępów po wykresach */
...
with st.sidebar:
    ...
    st.divider()
    st.caption("System: Magazyn v2.3")
...
if st.session_state.menu == "Pulpit Manedżerski":
    ...
    if not df.empty:
        ...
        st.info("Brak zarejestrowanych towarów w systemie.")

elif st.session_state.menu == "Wyszukiwarka Zasobów":
    st.title("Wyszukiwarka Zasobów")
    qs = st.text_input("Wprowadź nazwę towaru do odfiltrowania...")
    ...
    if kat_df.empty:
        ...
        with st.form("delivery"):
            c1, c2 = st.columns(2)
            nazwa = c1.text_input("Pełna nazwa artykułu")
            kat = c1.selectbox("Kategoria systemowa", kat_df['nazwa'])
            ilosc = c2.number_input("Ilość (szt.)", min_value=0, step=1)
            cena = c2.number_input("Cena netto (PLN)", min_value=0.0, step=0.01)
    ...
    if not df.empty:
        st.subheader("Zestawienie analityczne kosztów")
        # Wyświetlamy tylko tabelę bez wykresu area_chart
        st.table(df)
        
        c1, c2 = st.columns(2)
        total_val = df['suma'].sum()
        c1.metric("Całkowita wycena netto", f"{total_val:,.2f} PLN")
    else:
        st.warning("Brak danych finansowych do wyświetlenia.")
    ...
    with col_in:
        nowa_kat = st.text_input("Dodaj nową grupę")
        if st.button("Zapisz w rejestrze"):
            if nowa_kat:
                conn = get_connection()
